...
void RHD2000Editor::buttonEvent(Button* button)
{
	...
	if(fifoBtn->getToggleState()){
	    ...
	    Popen({"fifo", "5120000"}, input{"/dev/xillybus_spk_info_32"}, output{"./spk.bin"});
	    Popen({"fifo", "5120000"}, input{"/dev/xillybus_spk_wav_32"},  output{"./spk_wav.bin"});
...
void RHD2000Editor::saveCustomParameters(XmlElement* xml)
{
    ...
    xml->setAttribute("DAC_TTL", fifoBtn->getToggleState());
...
void RHD2000Editor::loadCustomParameters(XmlElement* xml)
{
    ...
    fifoBtn->setToggleState(xml->getBoolAttribute("RAM_FIFO"), sendNotification);
